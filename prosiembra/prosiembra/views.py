...
def comprar_producto(request, producto_id):
    ...
    if request.method == 'POST':
        cantidad_comprada = int(request.POST['cantidad'])
        if cantidad_comprada <= producto.cantidad_disponible:
            producto.cantidad_disponible -= cantidad_comprada
            producto.save()
            return redirect('products')
        else:
            return render(request, 'mensaje_de_error.html', {'mensaje': 'No hay suficiente inventario disponible.'})
